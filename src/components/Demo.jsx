    ...
    <section className='mt-16 w-full max-w-xl'>
      ...
      <div className='flex flex-col w-full gap-2'>
        <form className='relative flex justify-center items-center' onSubmit={handleSubmit} >
          ...
          <button type='submit' className='submit_btn' >S</button>
        </form>
        {/* Browse URL History */}
            ...
            {allArticles.map((item,index) => (
              <div key={`link-${index}`} onClick={() => setArticle(item)} className='link_card'>
                <div className='copy_btn'>
                  <img src={copied === item.url ? tick : copy} alt="copy_icon" onClick={() => handleCopy(item.url)} className='w-[40%] h-[40%] object-contain'/>
                </div>
      ...
      <div className='my-10 max-w-full flex justify-center items-center'>
        {isFetching? (
          ...
          <p className='font-inter font-bold text-black text-center'>Well, that wasnt supposed to happpen... <br />
